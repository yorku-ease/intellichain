...
def predicted_polygonscan(data):
    
    list_append = list()
    data = pd.DataFrame(data).transpose()
    list_append.append(data)
    data.columns = ['pendingcount','avgtxnsperblock','avgnetworkutilization']
    data['avgnetworkutilization'] = boxcox1p(data['avgnetworkutilization'] , 0.15)
    data['avgtxnsperblock'] = boxcox1p(data['avgtxnsperblock'] , 0.15)
    data['pendingcount'] = boxcox1p(data['pendingcount'] , 0.15)
    data = np.asarray(data)
    
    rapid_gas_scaler = joblib.load(path.join(dir, 'data_polygonscan_std_scaler.bin'))
    ...
    rapid_gas_lstm = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_rapid_lstm.h5'))
    rapid_gas_bilstm = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_rapid_bilstm.h5'))
    rapid_gas_gru = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_rapid_gru.h5'))
    ...
    rapid_gas_lstm_error = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_error_lstm.h5'))
    rapid_gas_gru_error = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_error_gru.h5'))
    rapid_gas_bilstm_error = load_model(path.join(dir, './Model_weights_polyganscan/data_polygonscan_error_bilstm.h5'))
    ...
    final_frame.to_csv('code_polygonscan.csv',index=None)
    

data = [8000,54,37.87]

predicted_polygonscan(data)